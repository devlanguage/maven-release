 ...
 * 
 */
public class ActiveMqServerPerformanceTest {
	/**
	 * This value is returned from the method <CODE>getAcknowledgeMode</CODE> if the
	 * session is transacted. If a <CODE>Session</CODE> is transacted, the
	...
	public static void main(String[] args) {
		// ActiveMQ: DeliveryMode.PERSISTENT
		// Localhost server: 2000 message per seconds by average
		// Remote server: 120 message per seconds by average

		// ActiveMQ: DeliveryMode.NON_PERSISTENT
		// Localhost server: 2000 message per seconds by average
		// Remote server: 2000 message per seconds by average
		try {
			Session jmsSession1 = MessageUtil.getInstance().getSession(JmsServerType.ACTIVE_MQ, false,
					javax.jms.Session.AUTO_ACKNOWLEDGE);
			Session jmsSession2 = MessageUtil.getInstance().getSession(JmsServerType.ACTIVE_MQ, false,
					javax.jms.Session.AUTO_ACKNOWLEDGE);

			String testQueueName = ActiveMqServerPerformanceTest.class.getName().replaceAll("\\.", "_");
				...
				public void onMessage(javax.jms.Message msg) {
					if (counter.getAndIncrement() % refSpot == 0) {
						if (counter.get() > 1) {
							...
							System.out.println(
									"send " + refSpot * 1000 / (currentTime - lastTime2) + " message per seconds");
							if (statisticsTimeCount != 0 && statisticsTimeCount % 10 == 0) {
								System.out.println(
										"average: " + (statisticsTimeCount * refSpot * 1000) / (currentTime - lastTime1)
												+ " message per seconds");
