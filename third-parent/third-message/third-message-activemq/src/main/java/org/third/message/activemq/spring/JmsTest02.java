...
//            });
		/*
		 * String queueAName = "ygong_dest_queue1"; javax.jms.Queue queueA =
		 * consumerSession.createQueue(queueAName); //
		 * consumerSession.setMessageListener(new MessageListener() { // // public void
		 * onMessage(javax.jms.Message msg) { // System.out.println(msg); // } // });
		 * 
		 * final javax.jms.MessageConsumer consumer01 =
		 * consumerSession.createConsumer(queueA); consumer01.setMessageListener(new
		 * MessageListener() {
		 * 
		 * public void onMessage(Message message) { System.err.println(message);
		 * 
		 * } }); // new Thread(new Runnable() { // // public void run() { // while
		 * (true) { // TextMessage textMsg; // try { // textMsg = (TextMessage)
		 * consumer01.receive(); // System.out.println(textMsg); // } catch
...
//        }
		 ...
		 * 
		 * ActiveMQResourceAdapter activeMqResourceAdapter = (ActiveMQResourceAdapter)
		 * ra;
		 * 
		 * try { System.out.println(activeMqResourceAdapter.makeConnection()); } catch
