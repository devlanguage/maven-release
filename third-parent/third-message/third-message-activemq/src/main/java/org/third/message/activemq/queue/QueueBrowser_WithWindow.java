package org.third.message.activemq.queue;
...
/**
 * The QBrowser example is a GUI application that lets you visually examine the contents of a JMS Queue. It is written
 * using javax.swing.
 * 
 * By default QBrowser will connect to the imqbrokerd running on localhost:7676. You can use -DimqAddressList attribute
 * to change the host, port and transport:
 * 
 * java -DimqAddressList=mq://<host>:<port>/jms QBrowser
 * 
 * Once QBrowser is up, enter the name of a queue and click Browse. A list of messages on the queue will appear in the
 * main window. Select a message and click "Details" to see the contents of the message.
 * 
 * QBrowser consists of the following classes:
 * 
 * QBrowser main(), the base GUI frame, and the JMS code MsgTable A TableModel for handling the display of messages on a
 * queue PropertyPanel A JPanel with a scrolling text area for displaying simple text, or the contents of a HashMap. A
 * number of minor event handling classes.
 * 
 */
@SuppressWarnings({ "serial", "null", "unchecked", "deprecation", "unused", "synthetic-access", "cast" })
public class QueueBrowser_WithWindow extends JPanel implements javax.jms.MessageListener {
    ...
    /**
     * Setup a consumer that listens on the Message Queue monitoring topic that sends out lists of destinations.
     */
    ...
    private void showDetails(Message msg, int msgno) {
        ...
        if (detailsFrame == null) {
            ...
            detailsFrame.setTitle(QueueBrowser_WithWindow.title + " - Message Details");
        ...
        try {
            for (Enumeration enu = msg.getPropertyNames(); enu.hasMoreElements();) {
        ...
        // Load message body
        bodyPanel.setTitle("Message Body: (" + QueueBrowser_WithWindow.messageType(msg) + ")");
    ...
    public final static void startQueueBrowser() {
        ...
        frame.setTitle(QueueBrowser_WithWindow.title + " - " + serverHost + ":" + serverPort);
        frame.setBackground(Color.white);
        frame.getContentPane().setLayout(new BorderLayout());
        frame.getContentPane().add("Center", new QueueBrowser_WithWindow());
        frame.pack();
        frame.show();
    }
    ...
    private static void usage() {
        ...
        System.out.println("usage: java QBrowser \n");
        System.exit(1);
    }
    ...
    /**
     * Return a string representation of the body of a JMS bytes message. This is basically a hex dump of the body.
    ...
    /**
     * Takes a buffer of bytes and returns a hex dump. Each hex digit represents 4 bits. The hex digits are formatted
     * into groups of 4 (2 bytes, 16 bits). Each line has 8 groups, so each line represents 128 bits.
     */
    private static String toHexDump(byte[] buf, int length) {
        ...
        /*
         * Assume buf is in network byte order (most significant byte is buf[0]). Convert two byte pairs to a short,
         * then display as a hex string.
         */
        int n = 0;
    ...
    /**
     * Consumer that listens on the MQ monitoring topic that sends out lists of destination names. We use this to update
     * the combo-box menu.
     */
    ...
    class MsgTable extends AbstractTableModel {
        ...
        final String[] columnNames = { "#", "Timestamp", "Type", "Mode", "Priority" };
        ...
        public Object getValueAt(int row, int column) {
            ...
            try {
                switch (column) {
                    ...
                    case 2:
                        return QueueBrowser_WithWindow.messageType(m);
                    case 3:
                        // Delivery mode
    ...
    /**
     * A panel with a text area that knows how to format and display a HashMap of values.
     */
    class PropertyPanel extends JPanel {
        ...
        /**
         * Pad a string to the specified width, right justified. If the string is longer than the width you get back the
         * original string.
         */
