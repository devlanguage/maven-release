...
public class RemoteQueue extends BaseQueue {
    ...
    private synchronized void initSendQueue() throws JMSException {
        ...
        if (sendSession == null) {
            sendSession = ImqMessageService.getInstance().getQueueSession();
        }
        if (queueSender == null) {
    ...
    private synchronized void initRecvQueue() throws JMSException {
        ...
        if (queueReceiver == null) {
            queueReceiver = ImqMessageService.getInstance().getQueueReceiver(this);
        }
    }
    ...
    private class ReplyQueueReceiverThread extends Thread {
        ...
        public void run() {
            ...
            try {
                QueueSession replyQueueSession = ImqMessageService.getInstance().getQueueSession();
