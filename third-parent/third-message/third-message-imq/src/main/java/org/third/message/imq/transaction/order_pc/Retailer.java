...
public class Retailer extends Thread {
	...
	/**
	 * Constructor. Instantiates the retailer with the quantity of computers being
	 * ordered.
	 */
	...
	public void run() {
		...
		try {
			...
			vendorOrderQueue = SampleUtilities.getQueue(TransactionOrderTest.A1_retailerOrderQueueName,
					nonTransactedSession);
			retailerConfirmationQueue = SampleUtilities.getQueue(TransactionOrderTest.A2_retailerConfirmationQueueName,
					nonTransactedSession);
		} catch (Exception e) {
		...
		/*
		 * Create non-transacted session and sender for vendor order queue. Create
		 * message to vendor, setting item and quantity values. Send message. Create
		 * receiver for retailer confirmation queue. Get message and report result. Send
		 * an end-of-message-stream message so vendor will stop processing orders.
		 */
		...
		try {
			...
			System.out.println("Retailer: ordered " + quantity + " computer(s) on " + TransactionOrderTest.A1_retailerOrderQueueName
					+ " and wait for result on " + TransactionOrderTest.A2_retailerConfirmationQueueName);
			...
			msgProducer.send(orderMapMsg);
			System.out.println("Retailer: ordered " + orderMapMsg.getInt("Quantity") + " computer(s)  on "
					+ TransactionOrderTest.A1_retailerOrderQueueName + " and wait for result on "
					+ TransactionOrderTest.A2_retailerConfirmationQueueName);
			confirmOrderMapMsg = (MapMessage) orderConfirmationConsumer.receive();
			if (confirmOrderMapMsg.getBoolean("OrderAccepted") == true) {
				System.out.println("Retailer: Order filled");
			} else {
