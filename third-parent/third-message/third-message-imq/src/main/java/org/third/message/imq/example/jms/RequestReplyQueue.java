...
public class RequestReplyQueue {
    ...
    public class Request extends Thread {
        ...
        public void run() {
            ...
            try {
                ...
                System.out.println("REQUEST: Sending message: " + message.getText());
    ...
    public class Reply extends Thread {
        ...
        public void run() {
            ...
            TextMessage          reqMsg = null; 
            Queue                tempQueue = null;
            MessageProducer      replyTempProducer = null;
            TextMessage          reply = null; 
...
reqMsg = (TextMessage) msgConsumer.receive();
System.out.println("REPLY: Message received: " + reqMsg.getText());
tempQueue = (Queue) reqMsg.getJMSReplyTo();
replyTempProducer = session.createProducer(tempQueue);
reply = session.createTextMessage();
reply.setText(REPLY_TEXT);
reply.setJMSCorrelationID(reqMsg.getJMSMessageID());
System.out.println("REPLY: Sending reply: " + reply.getText());
replyTempProducer.send(reply);
            } catch (JMSException e) {
                System.out.println("Exception occurred: " + e.toString());
                exitResult = 1;
            } catch (Exception ee) {
    ...
    public static void main(String[] args) {
        ...
        if (args.length != 1) {
            ...
            rrq.queueName = "RequestReplyQueue_Test";
    	} else{
