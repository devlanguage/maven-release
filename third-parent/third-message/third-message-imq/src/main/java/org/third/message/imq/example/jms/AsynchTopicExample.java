...
/**
 * The AsynchTopicExample class demonstrates the use of a message listener in the publish/subscribe
 * model. The producer publishes several messages, and the consumer reads them asynchronously.
 * <p>
 * The program contains a MultipleProducer class, an AsynchConsumer class with a listener class, a
 * main method, and a method that runs the consumer and producer threads.
 * <p>
...
public class AsynchTopicExample {
    ...
    /**
     * The AsynchConsumer class fetches several messages from a topic asynchronously, using a
    ...
    public class AsynchConsumer extends Thread {
         ...
         * method for the AsynchConsumer class.
         */
        private class TextListener implements MessageListener {
            ...
            public void onMessage(Message message) {
                ...
                if (message instanceof TextMessage) {
                    ...
                    try {
                        System.out.println("CONSUMER THREAD: Reading message: " + msg.getText());
        ...
        public void run() {
            ...
            /*
             * Create consumer. Register message listener (TextListener). Start message delivery.
             * Send synchronize message to producer, then wait till all messages have arrived.
            ...
            try {
                ...
                try {
                    SampleUtilities.sendSynchronizeMessage("CONSUMER THREAD: ", CONTROL_QUEUE);
                } catch (Exception e) {
                ...
                /*
                 * Asynchronously process messages. Block until producer issues a control message
    ...
    public class MultipleProducer extends Thread {
        ...
        public void run() {
            ...
            /*
             * After synchronizing with consumer, create producer. Create text message. Send
             * messages, varying text slightly. Send end-of-messages message. Finally, close
             * connection.
             */
            try {
                /*
                 * Synchronize with consumer. Wait for message indicating that consumer is ready to
                 * receive messages.
                 */
                try {
                    SampleUtilities.receiveSynchronizeMessages("PRODUCER THREAD: ", CONTROL_QUEUE,
                            1);
                } catch (Exception e) {
                    System.out.println("Queue probably missing: " + e.toString());
                    if (connection != null) {
                ...
                for (int i = 0; i < NUMMSGS; i++) {
                    ...
                    System.out.println("PRODUCER THREAD: Publishing message: " + message.getText());
    ...
    /**
     * Instantiates the consumer and producer classes and starts their threads. Calls the join
     * method to wait for the threads to die.
     * <p>
     * It is essential to start the consumer before starting the producer. In the publish/subscribe
     * model, a consumer can ordinarily receive only messages published while it is active.
     */
    ...
    public static void main(String[] args) {
        ...
        if (args.length != 1) {
            ...
            ate.topicName = SampleUtilities.readLine();
        } else {
