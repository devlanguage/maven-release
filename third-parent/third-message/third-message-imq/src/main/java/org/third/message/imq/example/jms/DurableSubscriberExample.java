...
/**
 * The DurableSubscriberExample class demonstrates that a durable subscription is active even when
 * the subscriber is not active.
 * <p>
 * The program contains a DurableSubscriber class, a MultiplePublisher class, a main method, and a
 * method that instantiates the classes and calls their methods in sequence.
 * <p>
 * The program begins like any publish/subscribe program: the subscriber starts, the publisher
 * publishes some messages, and the subscriber receives them.
 * <p>
 * At this point the subscriber closes itself. The publisher then publishes some messages while the
 * subscriber is not active. The subscriber then restarts and receives the messages.
 * <p>
...
public class DurableSubscriberExample {
    ...
    /**
     * The DurableSubscriber class contains a constructor, a startSubscriber method, a
     * closeSubscriber method, and a finish method.
     * <p>
     * The class fetches messages asynchronously, using a message listener, TextListener.
     */
    public class DurableSubscriber {
         ...
         * method for the DurableSubscriber class.
         */
        ...
        /**
         * Constructor: looks up a connection factory and topic and creates a connection and
         * session.
         */
        ...
        /**
         * Stops connection, then creates durable subscriber, registers message listener
         * (TextListener), and starts message delivery; listener displays the messages obtained.
         */
        public void startSubscriber() {
            ...
            try {
                ...
                connection.stop();
                topicSubscriber = session.createDurableSubscriber(topic, "MakeItLast");
        ...
        /**
         * Blocks until publisher issues a control message indicating end of publish stream, then
         * closes subscriber.
         */
        ...
        public void finish() {
            ...
            if (connection != null) {
                try {
                    ...
                    System.out.println("unsubscribe the subscription: MakeItLast");
                    connection.close();
                } catch (JMSException e) {
                    exitResult = 1;
                }
    ...
    /**
     * The MultiplePublisher class publishes several messages to a topic. It contains a constructor,
     * a publishMessages method, and a finish method.
     */
    public class MultiplePublisher {
        ...
        /**
         * Constructor: looks up a connection factory and topic and creates a connection and
         * session. Also creates the producer.
         */
        ...
        /**
         * Creates text message. Sends some messages, varying text slightly. Messages must be
         * persistent.
         */
     ...
     * 
     * Closes the subscriber; while it is closed, the publisher publishes some more messages.
     * 
    ...
    /**
     * Reads the topic name from the command line, then calls the run_program method.
     * 
     * @param args
    ...
    public static void main(String[] args) {
        ...
        if (args.length != 1) {
            ...
            dse.topicName = DurableSubscriberExample.class.getSimpleName();
        } else{
