...
/**
 * The AckEquivExample class shows how the following two scenarios both ensure that a message will
 * not be acknowledged until processing of it is complete:
 * <ul>
 * <li> Using an asynchronous consumer (message listener) in an AUTO_ACKNOWLEDGE session
 * <li> Using a synchronous consumer in a CLIENT_ACKNOWLEDGE session
 * </ul>
 * <p>
 * With a message listener, the automatic acknowledgment happens when the onMessage method returns --
 * that is, after message processing has finished.
 * <p>
 * With a synchronous receive, the client acknowledges the message after processing is complete. (If
 * you use AUTO_ACKNOWLEDGE with a synchronous receive, the acknowledgement happens immediately
 * after the receive call; if any subsequent processing steps fail, the message cannot be
 * redelivered.)
 * <p>
 * The program contains a SynchProducer class, a SynchConsumer class, an AsynchSubscriber class with
 * a TextListener class, a MultiplePublisher class, a main method, and a method that runs the other
 * classes' threads.
 * <p>
 * Specify a queue name and a topic name on the command line when you run the program. The program
 * also uses a queue named "controlQueue", which should be created before you run the program.
 */
public class AckEquivExample {
    ...
    /**
     * The SynchProducer class creates a session in CLIENT_ACKNOWLEDGE mode and sends a message.
     */
    public class SynchProducer extends Thread {
        ...
        public void run() {
            ...
            /*
             * Create client-acknowledge producer. Create and send message.
             */
            try {
    ...
    /**
     * The SynchConsumer class creates a session in CLIENT_ACKNOWLEDGE mode and receives the message
     * sent by the SynchProducer class.
     */
    public class SynchConsumer extends Thread {
        ...
        public void run() {
            ...
            /*
             * Create client-acknowledge consumer. Receive message and process it. Acknowledge
             * message.
             */
            try {
                ...
                System.out.println("CONSUMER: Processing message: " + message.getText());
    ...
    /**
     * The AsynchSubscriber class creates a session in AUTO_ACKNOWLEDGE mode and fetches several
     * messages from a topic asynchronously, using a message listener, TextListener.
     * <p>
    ...
    public class AsynchSubscriber extends Thread {
        ...
        /**
         * The TextListener class implements the MessageListener interface by defining an onMessage
         * method for the AsynchSubscriber class.
         */
        private class TextListener implements MessageListener {
            ...
            /**
             * Casts the message to a TextMessage and displays its text. A non-text message is
             * interpreted as the end of the message stream, and the message listener sets its
            ...
            public void onMessage(Message message) {
                ...
                if (message instanceof TextMessage) {
                    ...
                    try {
                        System.out.println("CONSUMER: Processing message: " + msg.getText());
                    } catch (JMSException e) {
        ...
        public void run() {
            ...
            /*
             * Create auto-acknowledge subscriber. Register message listener (TextListener). Start
             * message delivery. Send synchronize message to publisher, then wait till all messages
             * have arrived. Listener displays the messages obtained.
             */
            try {
                ...
                try {
                    SampleUtilities.sendSynchronizeMessage("CONSUMER: ", CONTROL_QUEUE);
                } catch (Exception e) {
                ...
                /*
                 * Asynchronously process messages. Block until publisher issues a control message
    ...
    /**
     * The MultiplePublisher class creates a session in AUTO_ACKNOWLEDGE mode and publishes three
     * messages to a topic.
     */
    public class MultiplePublisher extends Thread {
        ...
        public void run() {
            ...
            /*
             * After synchronizing with subscriber, create publisher. Send 3 messages, varying text
             * slightly. Send end-of-messages message.
             */
            try {
                /*
                 * Synchronize with subscriber. Wait for message indicating that subscriber is ready
                 * to receive messages.
                 */
                try {
                    SampleUtilities.receiveSynchronizeMessages("PRODUCER: ", CONTROL_QUEUE, 1);
                } catch (Exception e) {
                    System.out.println("Queue probably missing: " + e.toString());
                    if (connection != null) {
                        try {
                ...
                for (int i = 0; i < NUMMSGS; i++) {
                    ...
                    System.out.println("PRODUCER: Publishing message: " + message.getText());
    ...
    /**
     * Instantiates the producer, consumer, subscriber, and publisher classes and starts their
     * threads. Calls the join method to wait for the threads to die.
     */
    ...
    /**
     * Reads the queue and topic names from the command line, then calls the run_threads method to
     * execute the program threads.
     * 
     * @param args
    ...
    public static void main(String[] args) {
        ...
        if (args.length != 2) {
            ...
            aee.queueName = SampleUtilities.readLine();
            aee.topicName = SampleUtilities.readLine();
        }else{
