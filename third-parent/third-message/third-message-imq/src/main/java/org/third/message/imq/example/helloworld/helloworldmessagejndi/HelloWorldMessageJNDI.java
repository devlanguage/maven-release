package org.third.message.imq.example.helloworld.helloworldmessagejndi;
...
/*
 * Simple message producer and consumer which obtains its ConnectionFactory and Queue from
 * Administered Objects using a JNDI lookup.
 * 
 * This example requires two administered objects to be stored before it will work.
 * 
 * 1. A ConnectionFactory Object stored with a lookup name of ``MyConnectionFactory'' 2. A Queue
 * Object stored with a lookup name of ``MyQueue''
 * 
 * The initial context factory is configured in this example to use the File Store (an alternative
 * would be to use LDAP). The provider url (where the file store is located) is defined by default
 * to be C:/Temp This is a Windows specific path. When running this example on Unix, the first
 * command line parameter must be set to the Unix directory url where the File Store has been
 * created. e.g. file:///tmp on Solaris/Linux.
 * 
 * What this means is that an MQ Object Store should be created (via imqadmin or imqobjmgr) at
 * C:/Temp. When that store is created the following JNDI Naming Service Properties should be set
 * (via the imqadmin or imqobjmgr):
 * 
 * java.naming.provider.url = file:///C:/Temp java.naming.factory.initial =
 ...
 * 
 * If you have turned off auto-creation of destinations on the MQ broker then a physical Queue
 * destination matching the destination name used in the queue administered object needs to be
 * created on the broker. That is best accomplished by using the Administration Console.
 * 
 * For this example to compile and run the following must be in your CLASSPATH (in addition to the
...
public class HelloWorldMessageJNDI {
    ...
    public HelloWorldMessageJNDI(String url) {
        ...
        try {
            ...
            System.err
                    .println("Please make sure you have created the Connection Factory object using the command:");
        ...
        try {
            ...
            System.err
                    .println("Please make sure you have created the Queue object using the command:");
