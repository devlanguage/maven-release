package org.third.message.imq.example.jms;
 ...
 * <p>
 * The program contains a Publisher class, a Subscriber class with a listener class, a main method,
 * and a method that runs the subscriber and publisher threads.
 * <p>
 * The Publisher class publishes 30 messages of 6 different types, randomly selected, then publishes
 * a "Finished" message. The program creates four instances of the Subscriber class, one for each of
 * three types and one that listens for the "Finished" message. Each subscriber instance uses a
 * different message selector to fetch messages of only one type. The publisher displays the
 * messages it sends, and the listener displays the messages that the subscribers receive. Because
 * all the objects run in threads, the displays are interspersed when the program runs.
 * <p>
 * Specify a topic name on the command line when you run the program. The program also uses a queue
 * named "controlQueue", which should be created before you run the program.
 */
public class TopicSelectors {

    final String CONTROL_QUEUE = "controlQueue";
    String topicName = null;
    static final String MESSAGE_TYPES[] = { "Nation/World", "Metro/Region", "Business", "Sports",
            "Living/Arts", "Opinion",
            // always last type
            "Finished" };
    static final String END_OF_MESSAGE_STREAM_TYPE = MESSAGE_TYPES[MESSAGE_TYPES.length - 1];
    int exitResult = 0;

    /**
     * The Publisher class publishes a number of messages. For each, it randomly chooses a message
     * type. It creates a message and sets its text to a message that indicates the message type. It
     * also sets the client property NewsType, which the Subscriber objects use as the message
     * selector. After a pause to allow the subscribers to get all the messages, the publisher sends
     * a final message with a NewsType of "Finished", which signals the end of the messages.
     */
    public class Publisher extends Thread {
        ...
        /**
         * Chooses a message type by using the random number generator found in java.util.
         * 
        ...
        public void run() {
            ...
            /*
             * After synchronizing with subscriber, create publisher. Create and send news messages.
             * Send end-of-messages message.
             */
            try {
                /*
                 * Synchronize with subscribers. Wait for messages indicating that all subscribers
                 * are ready to receive messages.
                 */
                try {
                    SampleUtilities.receiveSynchronizeMessages("PUBLISHER THREAD: ", CONTROL_QUEUE,
                            NUM_SUBSCRIBERS);
                } catch (Exception e) {
    ...
    /**
     * Each instance of the Subscriber class creates a subscriber that uses a message selector that
     * is based on the string passed to its constructor. It registers its message listener, then
     * starts listening for messages. It does not exit until the message listener sets the variable
     * done to true, which happens when the listener gets the last message.
     */
    public class Subscriber extends Thread {
        ...
        /**
         * The MultipleListener class implements the MessageListener interface by defining an
         * onMessage method for the Subscriber class.
         */
        private class MultipleListener implements MessageListener {
            ...
            /**
             * Displays the message text. If the value of the NewsType property is "Finished", the
             * message listener sets its monitor state to all done processing messages.
             * 
             * @param inMessage
        ...
        /**
         * Constructor. Sets whatKind to indicate the type of message this Subscriber object will
         * listen for; sets subscriberNumber based on Subscriber array index.
         * 
         * @param str
         *            a String from the MESSAGE_TYPES array
         * @param num
         *            the index of the Subscriber array
         */
        ...
        public void run() {
            ...
            /*
             * Create subscriber with message selector. Start message delivery. Send synchronize
             * message to publisher, then wait till all messages have arrived. Listener displays the
             * messages obtained.
             */
            try {
                selector = new String("NewsType = '" + whatKind + "'" + " OR NewsType = '"
                        + END_OF_MESSAGE_STREAM_TYPE + "'");
                System.out.println("SUBSCRIBER " + subscriberNumber + " THREAD: selector is \""
                        + selector + "\"");
                msgConsumer = session.createConsumer(topic, selector, false);
                msgConsumer.setMessageListener(multipleListener);
                connection.start();

                // Let publisher know that subscriber is ready.
                try {
                    SampleUtilities.sendSynchronizeMessage("SUBSCRIBER " + subscriberNumber
                            + " THREAD: ", CONTROL_QUEUE);
                } catch (Exception e) {
                    System.out.println("Queue probably missing: " + e.toString());
                    if (connection != null) {
                        try {
                ...
                /*
                 * Asynchronously process appropriate news messages. Block until publisher issues a
                 * finished message.
                 */
    ...
    /**
     * Creates an array of Subscriber objects, one for each of three message types including the
     * Finished type, and starts their threads. Creates a Publisher object and starts its thread.
    ...
    /**
     * Reads the topic name from the command line, then calls the run_threads method to execute the
     * program threads.
     * 
     * @param args
