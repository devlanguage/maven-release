 ...
 * <p>
 * Set the <code>USE_JNDI</code> variable to true or false depending on whether your provider uses
 * JNDI.
 * <p>
...
public class SampleUtilities {
    ...
    /*
     * Define the System Property "USE_JNDI" true to use JNDI lookup() e.g. $JAVA -DUSE_JNDI=true
     * ....
     */
    ...
    /**
     * Returns a ConnectionFactory object. If provider uses JNDI, serves as a wrapper around
     * jndiLookup method. If provider does not use JNDI, substitute provider-specific code here.
     * 
    ...
    /**
     * Returns a QueueConnectionFactory object. If provider uses JNDI, serves as a wrapper around
     * jndiLookup method. If provider does not use JNDI, substitute provider-specific code here.
     * 
    ...
    /**
     * Returns a TopicConnectionFactory object. If provider uses JNDI, serves as a wrapper around
     * jndiLookup method. If provider does not use JNDI, substitute provider-specific code here.
     * 
     ...
     *             (or other exception) if name cannot be found
     */
    public static javax.jms.TopicConnectionFactory getTopicConnectionFactory() throws Exception {
    ...
    /**
     * Returns a Queue object. If provider uses JNDI, serves as a wrapper around jndiLookup method.
     ...
     *             (or other exception) if name cannot be found
     */
    public static javax.jms.Queue getQueue(String name, javax.jms.Session session) throws Exception {
    ...
    /**
     * Returns a Topic object. If provider uses JNDI, serves as a wrapper around jndiLookup method.
     * If provider does not use JNDI, substitute provider-specific code here.
     * 
     * @param name
     ...
     *             (or other exception) if name cannot be found
     */
    public static javax.jms.Topic getTopic(String name, javax.jms.Session session) throws Exception {
    ...
    /**
     * Creates a JNDI InitialContext object if none exists yet. Then looks up the string argument
    ...
    /**
     * Wait for 'count' messages on controlQueue before continuing. Called by a publisher to make
     * sure that subscribers have started before it begins publishing messages.
     * <p>
    ...
    public static void receiveSynchronizeMessages(String prefix, String controlQueueName, int count)
        ...
        try {
            System.out.println(prefix + "Receiving synchronize messages from " + controlQueueName
                    + "; count = " + count);
            queueReceiver = queueSession.createReceiver(controlQueue);
            while (count > 0) {
                queueReceiver.receive();
                count--;
                System.out.println(prefix + "Received synchronize message; expect " + count
                        + " more");
            }
