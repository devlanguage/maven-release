...
begin
	for l_obj IN (SELECT * FROM user_objects WHERE object_name like ('H3_%') )
	loop
    IF l_obj.object_type='TABLE' THEN
      execute immediate 'drop '||l_obj.object_type||' '|| l_obj.object_name ||' cascade constraints';
    ELSE 
      execute immediate 'drop '||l_obj.object_type||' '|| l_obj.object_name;
    END IF;
	END loop;
END;
/                            
...
CREATE table h3_person (
	...
	"AGE" NUMBER(5,0),  
	"SEX" VARCHAR2(10) DEFAULT 'FEMALE',
...
ALTER TABLE H3_PERSON ADD CONSTRAINT H3_PERSON_CHK_AGE CHECK (AGE BETWEEN 0 AND 100) ENABLE;
ALTER TABLE H3_PERSON ADD CONSTRAINT H3_PERSON_UK_NAME UNIQUE (FIRST_NAME, LAST_NAME) ENABLE;
...
create sequence h3_seq_department;
...
BEGIN
  ...
  FOR l_dep IN (
      select dep_id,dep_name,logo,description,balance from h3_department
  )
  LOOP
      dbms_output.put_line('h3_department:dep_id='||l_dep.dep_id || ',dep_name='||l_dep.dep_name || ',balance='||l_dep.balance);
  END lOOP;
  
  dbms_output.put_line('Orignal Result: ');  
  OPEN v_ref_cursor FOR 'SELECT * FROM h3_department WHERE dep_id = :dep_id ' using v_dep_id;  
  LOOP
  	 FETCH v_ref_cursor INTO v_dep;
  	 EXIT WHEN v_ref_cursor%NOTFOUND;  	 
     dbms_output.put_line('h3_department:dep_id='||v_dep.dep_id || ',dep_name='||v_dep.dep_name || ',balance='||v_dep.balance);     
  END LOOP;
  CLOSE v_ref_cursor;
  
  dbms_output.put_line('Single Line Query: ');
  EXECUTE IMMEDIATE 'SELECT * FROM h3_department WHERE dep_id = :dep_id' INTO v_dep USING v_dep_id;
  dbms_output.put_line('h3_department:dep_id='||v_dep.dep_id || ',dep_name='||v_dep.dep_name || ',balance='||v_dep.balance);
  
  dbms_output.put_line('Update: ');  
  EXECUTE IMMEDIATE 'UPDATE h3_department SET balance = 1.5 * balance WHERE dep_id = :dep_id RETURNING balance INTO :balance' USING v_dep_id RETURNING INTO v_balance;
  ...
  END LOOP;
  CLOSE v_ref_cursor;
END H3_LIST_DEPARTMENT;
/
create or replace
PROCEDURE H3_ADD_DEPARTMENT(p_department_name VARCHAR2) 
AS
BEGIN
  INSERT INTO h3_department(dep_id, dep_name) VALUES(h3_seq_department.nextval, p_department_name);
END H3_ADD_DEPARTMENT;
/
...
END H3_EXCEPTION_TEST;
/



create or replace
procedure h3_update_dep_xml
AS
