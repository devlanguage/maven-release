...
public class PortForwarding {
    ...
    public static void main(String[] args) {
        ...
        try {
             ...
             * 
             * This works by allocating a socket to listen on 8080 on the local interface (127.0.0.1). Whenever a
             * connection is made to this port (127.0.0.1:8080), the connection is forwarded over the secure channel,
             * and a connection is made to www.ethz.ch:80 from the remote machine (i.e., the ssh server).
             * 
             ...
             * 
             * Specifies that the port 127.0.0.1:8080 on the remote server is to be forwarded to the given host and port
             * on the local side. This works by allocating a socket to listen to port 8080 on the remote side (the ssh
             * server), and whenever a connection is made to this port, the connection is forwarded over the secure
             * channel, and a connection is made to www.ganymed.ethz.ch:80 by the Ganymed SSH-2 library.
             * 
             * (the above text is based partially on the OpenSSH man page)
             */
            ...
            /*
             * Now, on the ssh server, if you connect to 127.0.0.1:8080, then the connection is forwarded through the
             * secure tunnel to the library, which in turn will forward the connection to www.ganymed.ethz.ch:80.
             */
