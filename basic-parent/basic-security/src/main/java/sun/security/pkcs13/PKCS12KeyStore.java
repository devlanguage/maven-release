 ...
 * Implements the PKCS#12 PFX protected using the Password privacy mode. The
 * contents are protected using Password integrity mode.
 *
 ...
 * --------------------------------------------------------------------- MSIE4
 * (domestic 40 bit RC2. 40 bit RC2 and xport versions) PKCS#12 export.
 *
 * MSIE4, 5 (domestic 40 bit RC2, 40 bit RC2, and export versions) 3 key triple
 * DES 3 key triple DES PKCS#12 import.
 *
 * MSIE5 40 bit RC2 3 key triple DES, PKCS#12 export. with SHA1 (168 bits)
 *
 * Netscape Communicator 40 bit RC2 3 key triple DES, (domestic and export with
 * SHA1 (168 bits) versions) PKCS#12 export
 *
 * Netscape Communicator 40 bit ciphers only All. (export version) PKCS#12
 * import.
 *
 * Netscape Communicator All. All. (domestic or fortified version) PKCS#12
 * import.
 *
 ...
 * certificate chain and aliases. In a PKCS12 keystore, entries are identified
 * by the alias, and a localKeyId is required to match the private key with the
 * certificate. Trusted certificate entries are identified by the presence of an
...
public final class PKCS12KeyStore extends KeyStoreSpi {
	...
	/**
	 * Private keys and certificates are stored in a map. Map entries are keyed by
	 * alias names.
	 */
	 ...
	 *         not identify a <i>key entry</i>.
	 *
	 * @exception NoSuchAlgorithmException  if the algorithm for recovering the key
	 *                                      cannot be found
	...
	public Key engineGetKey(String alias, char[] password) throws NoSuchAlgorithmException, UnrecoverableKeyException {
		...
		try {
			...
			/*
			 * Parse the key algorithm and then use a JCA key factory to re-create the key.
			 */
			...
			if (entry instanceof PrivateKeyEntry) {
				...
				if (debug != null) {
					debug.println("Retrieved a protected private key (" + key.getClass().getName() + ") at alias '"
							+ alias + "'");
				}
				...
				if (debug != null) {
					debug.println("Retrieved a protected secret key (" + key.getClass().getName() + ") at alias '"
							+ alias + "'");
				}
			}
		} catch (Exception e) {
	 ...
	 *         not exist or does not contain a certificate chain (i.e., the given
	...
	public Certificate[] engineGetCertificateChain(String alias) {
		...
		if (entry != null && entry instanceof PrivateKeyEntry) {
			if (((PrivateKeyEntry) entry).chain == null) {
				...
				if (debug != null) {
					debug.println("Retrieved a " + ((PrivateKeyEntry) entry).chain.length
	 ...
	 *
	 * <p>
	 * If the given alias name identifies a <i>trusted certificate entry</i>, the
	 * certificate associated with that entry is returned. If the given alias name
	 * identifies a <i>key entry</i>, the first element of the certificate chain of
	 * that entry is returned, or null if that entry does not have a certificate
	 * chain.
	 *
	 * @param alias the alias name
	 *
	 ...
	 * @param alias the alias name
	 *
	 * @return the creation date of this entry, or null if the given alias does not
	 *         exist
	 */
	 ...
	 *
	 * <p>
	 * If the given key is of type <code>java.security.PrivateKey</code>, it must be
	 ...
	 *
	 * @exception KeyStoreException if the given key cannot be protected, or this
		...
		try {
			...
			if (key instanceof PrivateKey) {
				...
				if ((key.getFormat().equals("PKCS#8")) || (key.getFormat().equals("PKCS8"))) {
					...
					if (debug != null) {
						debug.println("Setting a protected private key (" + key.getClass().getName() + ") at alias '"
								+ alias + "'");
					}
	 ...
	 * parse Algorithm Parameters
	 */
	private AlgorithmParameters parseAlgParameters(ObjectIdentifier algorithm, DerInputStream in) throws IOException { // 1.2.840.113549.1.5.13
		AlgorithmParameters algParams = null;
		try {
			DerValue params;
			if (in.available() == 0) {
	...
	/*
	 * Encrypt private key using Password-based encryption (PBE) as defined in
	 * PKCS#5.
	 *
	 * NOTE: By default, pbeWithSHAAnd3-KeyTripleDES-CBC algorithmID is used to
	 ...
	 *
	 * @exception KeyStoreException if the given alias already exists and does not
	 *                              identify a <i>trusted certificate entry</i>, or
	 ...
	 * If the entry being considered is a <i>trusted certificate entry</i>, the
	 * given certificate is compared to that entry's certificate. If the entry being
	 * considered is a <i>key entry</i>, the given certificate is compared to the
	...
	/**
	 * Gets a <code>KeyStore.Entry</code> for the specified alias with the specified
	 * protection parameter.
	 *
	 * @param alias     get the <code>KeyStore.Entry</code> for this alias
	 * @param protParam the <code>ProtectionParameter</code> used to protect the
	 *                  <code>Entry</code>, which may be <code>null</code>
	 *
	 * @return the <code>KeyStore.Entry</code> for the specified alias, or
	 ...
	 *                                        <code>protParam</code> were
	 *                                        insufficient or invalid
	 * @exception UnrecoverableKeyException   if the entry is a
	 *                                        <code>PrivateKeyEntry</code> or
	 *                                        <code>SecretKeyEntry</code> and the
	 *                                        specified <code>protParam</code> does
	...
	/**
	 * Saves a <code>KeyStore.Entry</code> under the specified alias. The specified
	 * protection parameter is used to protect the <code>Entry</code>.
	 *
	 * <p>
			...
			if (trustedKeyUsageValue != null) {
				if (trustedKeyUsageValue.length == 1) { // omit brackets
					entry.attributes.add(
							new PKCS12Attribute(TrustedKeyUsage_OID.toString(), trustedKeyUsageValue[0].toString()));
				} else { // multi-valued
					entry.attributes.add(
	 ...
	 *
	 * Hash-based MAC algorithm combines secret key with message digest to create a
	 ...
	 *
	 * Although attributes are optional, they could be required. For e.g. localKeyId
	 * attribute is required to match the private key with the associated end-entity
	 * certificate. The trustedKeyUsage attribute is used to denote a trusted
	 * certificate.
	 *
	 * PKCS8ShroudedKeyBags include unique localKeyID and friendlyName. CertBags may
	 * or may not include attributes depending on the type of Certificate. In
	 * end-entity certificates, localKeyID should be unique, and the corresponding
	 * private key should have the same localKeyID. For trusted CA certs in the
	 * cert-chain, localKeyID attribute is not required, hence most vendors don't
	 * include it. NSS/Netscape require it to be unique or null, where as IE/OpenSSL
	 * ignore it.
	 *
	 ...
	 * -------------------------------------------------------------- LocalKeyId (In
	 * EE cert only, NULL in CA certs) true true true true
	 *
	 * friendlyName unique same/ same/ unique unique unique/ null trustedKeyUsage -
	 * - - true
	 *
	 * Note: OpenSSL adds friendlyName for end-entity cert only, and removes the
	 * localKeyID and friendlyName for CA certs. If the CertBag did not have a
	 * friendlyName, most vendors will add it, and assign it to the DN of the cert.
	 */
	 ...
	 * Includes certificates in individual SafeBags of type CertBag. Each CertBag
	 * may include pkcs12 attributes (see comments in getBagAttributes)
	 */
	...
	/*
	 * Create SafeContent Data content type. Includes encrypted secret key in a
	 * SafeBag of type SecretBag. Includes encrypted private key in a SafeBag of
	 * type PKCS8ShroudedKeyBag. Each PKCS8ShroudedKeyBag includes pkcs12 attributes
	...
	/*
	 * Encrypt the contents using Password-based (PBE) encryption as defined in PKCS
	 * #5.
	 *
	 * NOTE: Currently pbeWithSHAAnd40BiteRC2-CBC algorithmID is used to derive the
	 * key and IV.
	 *
	 ...
	 *
	 * <p>
	 * If a password is given, it is used to check the integrity of the keystore
	 * data. Otherwise, the integrity of the keystore is not checked.
	 *
	 ...
	 *                                     the keystore data
	 * @exception NoSuchAlgorithmException if the algorithm used to check the
	...
	private void loadSafeContents(DerInputStream stream, char[] password)
		...
		for (int i = 0; i < count; i++) {
			...
			/*
			 * As per PKCS12 v1.0 friendlyname (alias) and localKeyId (keyId) are optional
			 * PKCS12 bagAttributes. But entries in the keyStore are identified by their
			 * alias. Hence we need to have an Unfriendlyname in the alias, if alias is
			 * null. The keyId attribute is required to match the private key with the
			 * certificate. If we get a bagItem of type KeyEntry with a null keyId, we
