/*    +--many-to-many <==================>  many-to-many-+ 			  ___________________
 ...
 |  SOURCE     |      | *PERSON_ID       | <--> | *PERSON_ID  |   	  ___________________
...
--   property list <======> blank   |			set,|one-to-many |   |___________________| 
       			 ...
       			 |dm_person_address|	 | dm_Magazine   |                 |
				     |_________________|   |_______________|       __________|________
			       ...
			       |   * city        | 	 |  price        |		|___________________|
					   ...
					   |_________________|	 |  title        | 		|   emblement_name	|
						  					           |* person_id    |		|   output			|
              	 										 ----------------		|   unit 			|
										                      							| * person_id       |	
                       																	--------------------
...
begin
	for l_tables IN (SELECT * FROM user_tables WHERE table_name like ('dm_%') )
	loop
...
COMMENT ON COLUMN "dm_PERSON"."PROFESSION" IS 'It identify what person is working on, "WORKER", "PEASANT","PERSON"';
COMMENT ON COLUMN "dm_PERSON"."EMBLEMENT" IS 'wheat, rice, broomcorn, oat,barley, soybean sprout';
...
create or replace
PROCEDURE dm_ADD_DEPARTMENT(p_department_name VARCHAR2) 
AS
BEGIN
...
CREATE OR REPLACE
PROCEDURE dm_EXCEPTION_TEST AS
