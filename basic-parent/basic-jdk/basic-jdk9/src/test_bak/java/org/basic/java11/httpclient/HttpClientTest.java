...
public class HttpClientTest {
    ...
    public void shouldReturnSampleDataContentWhenConnectViaSystemProxy()
        ...
        HttpRequest request = HttpRequest.newBuilder().uri(new URI("https://postman-echo.com/post"))
                ...
                .POST(HttpRequest.BodyPublishers.ofString("Sample body")).build();
    ...
    public void shouldReturnOKStatusForAuthenticatedAccess()
        ...
        HttpRequest request = HttpRequest.newBuilder().uri(new URI("https://postman-echo.com/basic-auth")).GET()
                .build();
    ...
    public void shouldUseJustTwoThreadWhenProcessingSendAsyncRequest()
        ...
        CompletableFuture<HttpResponse<String>> response1 = HttpClient.newBuilder().executor(executorService).build()
                .sendAsync(request, HttpResponse.BodyHandlers.ofString());

        CompletableFuture<HttpResponse<String>> response2 = HttpClient.newBuilder().executor(executorService).build()
                .sendAsync(request, HttpResponse.BodyHandlers.ofString());

        CompletableFuture<HttpResponse<String>> response3 = HttpClient.newBuilder().executor(executorService).build()
                .sendAsync(request, HttpResponse.BodyHandlers.ofString());
    ...
    public void shouldNotStoreCookieWhenPolicyAcceptNone()
        ...
        HttpClient httpClient = HttpClient.newBuilder().cookieHandler(new CookieManager(null, CookiePolicy.ACCEPT_NONE))
                .build();

        httpClient.send(request, HttpResponse.BodyHandlers.ofString());

        assertThat(((CookieManager) httpClient.cookieHandler().get()).getCookieStore().getCookies(), empty());
    }
    ...
    public void shouldStoreCookieWhenPolicyAcceptAll() throws URISyntaxException, IOException, InterruptedException {
        ...
        HttpClient httpClient = HttpClient.newBuilder().cookieHandler(new CookieManager(null, CookiePolicy.ACCEPT_ALL))
                .build();

        httpClient.send(request, HttpResponse.BodyHandlers.ofString());

        assertThat(((CookieManager) httpClient.cookieHandler().get()).getCookieStore().getCookies(), not(empty()));
    }
