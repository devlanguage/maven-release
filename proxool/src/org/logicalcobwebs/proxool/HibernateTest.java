 ...
 * <a href="http://www.hibernate.org">Hibernate 2.x</a>. (Code contributed by
 * Mark Eagle)
 * 
...
public class HibernateTest extends AbstractProxoolTest {
	...
	/**
	 * Can we get a connection straight from Hibernate? We register the pool
	 * first and theb ask for Hibernate for it.
	 * 
	...
	public void testSimpleHibernateConnection() throws HibernateException, ProxoolException {
		...
		String url = TestHelper.buildProxoolUrl(alias, TestConstants.HYPERSONIC_DRIVER,
		...
		// We just need to test that we served at least one connection. I
		// suspect that
		// Hibernate is doing its own house keeping and getting at least an
		// additional
		// one.
		assertTrue("servedCount", ProxoolFacade.getSnapshot(alias).getServedCount() > 0);
		// They should definitely all be returned to the pool once we're
		// finished though
	...
	/**
	 * Can we get a connection from a Proxool pool that we have already
	 * registered? We ask Hibernate to lookup the pool by its alias.
	 * 
	 * @throws ProxoolException
	 *             if there was a Proxool problem
	 * @throws HibernateException
	...
	public void testHibernateConfiguredConnection() throws HibernateException, ProxoolException {
		...
		try {
			hibernateProperties.setProperty(Environment.PROXOOL_XML,
					"src/java-test/org/logicalcobwebs/proxool/hibernate.xml");
			hibernateProperties.setProperty(Environment.PROXOOL_POOL_ALIAS, alias);
...
/*
 * Revision history: $Log: HibernateTest.java,v $ Revision 1.1 2015/06/30
 * 06:16:29 ygong empty log message ***
 * 
 * Revision 1.3 2006/03/24 00:14:59 billhorsman Changes for Hibernate 3
 * 
 * Revision 1.2 2006/01/18 14:40:06 billhorsman Unbundled Jakarta's Commons
 * Logging.
 * 
 * Revision 1.1 2003/09/28 09:38:30 billhorsman New unit test for Hibernate.
 * 
 */
