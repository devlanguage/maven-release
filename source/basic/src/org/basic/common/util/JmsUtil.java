 ...
 *  Last modifed on 4:40:51 PM Mar 10, 2014
 *
 *****************************************************************************
 */
 ...
 */
public class JmsUtil implements Runnable {
    ...
    static {
        Runtime.getRuntime().addShutdownHook(new Thread(new JmsUtil()));
    }
    static final CommonLogger logger = CommonLogger.getLog(JmsUtil.class);
    ...
    com.sun.messaging.ConnectionFactory imqConnFactory;
    ...
    private static final JmsUtil instance = new JmsUtil();

    public static final JmsUtil getInstance() {
    ...
    public Connection getConnection(JmsServerType jmsType, boolean durable, String clientID) throws JMSException {
        ...
        jmsConn = jmsType == JmsServerType.IMQ ? imqConnFactory.createConnection() : activeMqConnectionFactory.createConnection();
        // 如果你要使用DurableSubScription 方式,你必须为connection设置一个ClientID

        if (durable) {
            if (SystemUtil.isNullOrBlank(clientID)) {
                clientID = "ClientID_" + java.util.UUID.randomUUID().toString().replaceAll("-", "_");

            }
            jmsConn.setClientID(clientID);
        }
        SystemUtil.printMessage(toString(jmsConn));
    ...
    public javax.jms.Session createSession(JmsServerType jmsType, boolean transaction, int transactionMode) throws JMSException {
        return getSession(jmsType, transaction, transactionMode);
    }

    public javax.jms.Session getSession(JmsServerType jmsType, boolean transaction, int transactionMode) throws JMSException {
        initJmsConnectionFactory(jmsType);
        if (jmsType == JmsServerType.ACTIVE_MQ) {
            ...
            SystemUtil.printMessage(toString(activeMqConnection));
            ...
            SystemUtil.printMessage(toString(imqConnection));
            imqConnection.start();
            connections.add(imqConnection);

        }
        return jmsType == JmsServerType.ACTIVE_MQ ? activeMqConnection.createSession(transaction, transactionMode) : imqConnection
                        .createSession(transaction, transactionMode);
    }
    ...
    public String toString(javax.jms.ConnectionMetaData metaData) throws JMSException {
        ...
        return "ConnectionMetaData: [Name=" + metaData.getJMSProviderName() + "],ProviderVersion=" + metaData.getProviderVersion()
                        + "[ProviderMinorVersion=" + metaData.getProviderMinorVersion() + "[ProviderMinorVersion="
                        + metaData.getProviderMinorVersion() + "[JmsVersion=" + metaData.getJMSVersion() + ",JmsMajorVersion="
                        + metaData.getJMSMajorVersion() + ",JmsMinorVersion=" + metaData.getJMSMinorVersion() + "]";
    }
    ...
    @Override
    public void run() {
        for (javax.jms.Connection conn : connections) {
