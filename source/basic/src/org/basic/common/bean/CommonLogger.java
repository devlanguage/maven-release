...
public class CommonLogger implements org.apache.commons.logging.Log {
...
//    }
    ...
    // public static class CommonApacheLogger extends org.apache.commons.logging.impl.Log4JLogger {
    // }

    public static final int OFF = Level.OFF_INT;
    public static final int FATAL = Level.FATAL_INT;
    public static final int SEVERE = Level.ERROR_INT;
...
//        System.setProperty("org.apache.cxf.Logger", CommonLogger.CommonJdkLogger.class.getName());
    ...
    private static final CommonLogger nmaProfilingLogger = getCommonLogger("com.tellabs.ManagerPlatform.nma.Util.timer");
    private static final CommonLogger soapRequestLogger = getCommonLogger("com.tellabs.ManagerPlatform.nma.log.NMALoggingInInterceptor");
    private static final CommonLogger soapResponseLogger = getCommonLogger("com.tellabs.ManagerPlatform.nma.log.NMALoggingOutInterceptor");
    ...
    public boolean isTraceEnabled() {
        ...
        return logger.getLoggerRepository().isDisabled(Level.TRACE_INT) ? false : Level.TRACE.isGreaterOrEqual(logger.getEffectiveLevel());
    }

    public boolean isDebugEnabled() {

        return logger.getLoggerRepository().isDisabled(Level.DEBUG_INT) ? false : Level.DEBUG.isGreaterOrEqual(logger.getEffectiveLevel());
    }

    public boolean isInfoEnabled() {

        return logger.getLoggerRepository().isDisabled(Level.INFO_INT) ? false : Level.INFO.isGreaterOrEqual(logger.getEffectiveLevel());
    }

    public boolean isWarnEnabled() {
        return logger.getLoggerRepository().isDisabled(Level.WARN_INT) ? false : Level.WARN.isGreaterOrEqual(logger.getEffectiveLevel());
    }

    public boolean isErrorEnabled() {
        return logger.getLoggerRepository().isDisabled(Level.ERROR_INT) ? false : Level.ERROR.isGreaterOrEqual(logger.getEffectiveLevel());
    }

    public boolean isFatalEnabled() {
        return logger.getLoggerRepository().isDisabled(Level.FATAL_INT) ? false : Level.FATAL.isGreaterOrEqual(logger.getEffectiveLevel());
    }
     ...
     * 
     * This function is intended to lessen the computational cost of disabled log debug statements. If you check before
     * logging, you will not incur the cost of parameter construction. So, the preferred way for logging debug messages
     * is as follows.
     * <p>
     * if(logger.isDebugEnabled()) { logger.log(NbiLogger.DEBUG, "Common", "This is entry number: " + i ); }
     * 
     ...
     * 
     * @param level
     *            logging level defined in NbiLogger
     * @param target
     *            target area where logging happens, could be service name
     * @param message
     *            logging message
     */
    private void log(int level, Object target, Object message, Throwable t, boolean important) {
        if (important) {
            if (null == target) {
                message = " \n*************  " + message + " *************";
            } else {
        ...
        Priority logLevel = Level.toLevel(level, logger.getLevel());
        if (null == t) {
    ...
    public static final String listLoggers() {
        String defaultLoggerLevel = Logger.getRootLogger().getLevel().toString();
