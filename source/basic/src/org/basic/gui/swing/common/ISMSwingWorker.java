 ...
 * @version 1.0
 */
public abstract class ISMSwingWorker<P, R> {
    private R result;
     ...
     */
    public ISMSwingWorker(JRootPane theRootPane) {
     ...
     */
    public ISMSwingWorker(RootPaneContainer rootPaneParent) {
...

    {
        ...
        Runnable runWorker = new Runnable() {
            public void run() {
                try {
                    result = invokeBusyCommandHandler(obj);
                } catch (Exception ex) {
                    // ex.printStackTrace();
    ...
    private void setLocked(boolean lock) {
        ...
        if (lock) {
            focusOwner = lockGlassPane(glassPane);
        } else {
            unlockGlassPane(glassPane, focusOwner);
        }
    }
     ...
     *            boolean
     */
    private void setLocked(boolean lock, boolean unlocked) {
        // deal with glass pane
        JComponent glassPane = (JComponent) rootPane.getGlassPane();
        if (lock) {
