...
public class IPSeeker {
    ...
    public final static String QQwry_dat = "IP_Location.Dat";// "QQwry.dat";
    private static final String IP_FILE = IPSeeker.class.getResource(QQwry_dat).toString().substring(5);
    // 一些固定常量，比如记录长度等等
    ...
    /**
     * 从 offset位置读取4个字节为一个long，因为java为big-endian格式，所以没办法 用了这么一个函数来做转换
     * 
     * @param offset
    ...
    /**
     * 从 offset位置读取3个字节为一个long，因为java为big-endian格式，所以没办法 用了这么一个函数来做转换
     * 
     * @param offset
     * @return 读取的long值，返回-1表示读取文件失败
     */
    ...
    /**
     * 从 offset位置读取四个字节的ip地址放入ip数组中，读取后的ip为big-endian格式，但是 文件中是little-endian形式，将会进行转换
     * 
     * @param offset
     * @param ip
     */
