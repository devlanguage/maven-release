...
public class GraphDij {
    ...
    public void Shortest_Paths(Node s) {
        ...
        source = s; /*-*/
        alg.frame.Highlight(4);
        set = new Vector();
        PQ = new PriorityQueue(graph.node_cnt);
        Initialise(source);/*-*/
        alg.frame.Highlight(7);
                ...
                if (PQ.Exist_In_PQ(j + 1)) {
                    ...
                    if ((cost = CM.Cost(id - 1, j)) > 0.00) {
                        ...
                        if (temp_cost <= p.Cost()) {
                            ...
                            // System.out.println(" "+adj+" "+p.Get_ID()+"
                            // "+distance+" "+temp_cost);
                        }
                    }
                }
    ...
    private Edge Unique_Edge(Node n) {
        ...
        // System.out.println("here: highlight e.from =
        // "+e.From_Node().Get_ID()+", e.to = "+e.To_Node().Get_ID());
    ...
    private void Print_Ans() {
        ...
        /*
         * for (i = 0; i < 10; i++) { //System.out.println(" "+CM.Cost(i,0)+"
         * "+CM.Cost(i,1)+" "+CM.Cost(i,2)+" "+CM.Cost(i,3)+" "+CM.Cost(i,4)+"
         * "+CM.Cost(i,5)+" "+CM.Cost(i,6)+" "+CM.Cost(i,7)+" "+CM.Cost(i,8)+"
         * "+CM.Cost(i,9)); }
         */
        // PQ.Print_Val();
