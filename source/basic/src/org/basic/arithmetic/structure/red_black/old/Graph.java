package org.basic.arithmetic.structure.red_black.old;
...
import org.basic.arithmetic.structure.red_black.Arrow;
import org.basic.arithmetic.structure.red_black.Edge;
import org.basic.arithmetic.structure.red_black.Node;
...
public class Graph {
    ...
    public Graph(int n_nodes, int n_edges, int arrow_status) {
        ...
        // System.out.println( "New Graph " + n_nodes + " nodes, " + n_edges + "
        // edges" );
        n_arrows = n_edges;
    ...
    /*
     * public void Ani_Edge(Graphics g, int from, int to, int val) { int i, j,
     * k; Color change_to; int start_id, end_id, num_arcs = 1;
     * 
     * change_to = Color.black; for(i = 0; i < edge_cnt; i++) { if
     * (edges[i].Get_Start() == from && edges[i].Get_End() == to) { switch(val) {
     * case 1: change_to = Color.black; //original break; case 2: change_to =
     * Color.yellow; //intermediate break; case 3: change_to = Color.red;
     * //final break; default: break; }
     * 
     * start_id = from; end_id = to; for( k = 0; k<edge_cnt; k++) {
     * 
     * if (start_id == edges[k].Get_End() && end_id == edges[k].Get_Start()) {
     * num_arcs = 2; } }
     * 
     * 
     * edges[i].Highlight(g, change_to, num_arcs, arrow_status); break; } } }
     */
    /*
     * public void Ani_EdgeEndPoints( Graphics g, Edge e, int val ) { Node s,
     * end; s = e.From_Node(); end = e.To_Node(); Ani_Edge( g, s.Get_ID(),
     * end.Get_ID(), val ); Ani_Node( g, s.Get_ID(), val ); Ani_Node( g,
     * end.Get_ID(), val ); }
     */
    /*
     * public void Ani_Node(Graphics g, int n_id, int val) { int i; Color
     * change_to;
     * 
     * change_to = Color.blue; for (i = 0; i < node_cnt; i++) { if
     * (nodes[i].Get_ID() == n_id) { switch(val) { case 1: change_to =
     * Color.blue; //original break; case 2: change_to = Color.yellow;
     * //intermediate break; case 3: change_to = Color.red; //final break;
     * default: break; } nodes[i].Highlight(g, change_to); break; } } }
     */
