...
import org.basic.arithmetic.data.AlgAnimFrame;
import org.basic.arithmetic.data.ComBox;
import org.basic.arithmetic.data.DrawingObj;
import org.basic.arithmetic.data.DrawingPanel;
import org.basic.arithmetic.data.ShadowLabel;
...
class OptMatMult extends Object {
    ...
    /**
     * Dimension is used to represent the dimension of the matrices, where the
     * number of columns is represented by the 'width' attribute and the number
     * of rows is represented by the 'height' attribute from java.awt.Dimension
     */
    public OptMatMult(Dimension[] dims, IntMatrix costMat, IntMatrix bestMat, DrawingPanel drawingPanel,
            AlgAnimFrame frame, IntMatrix dimMat) {
    ...
    public void matEvaluate(int numMat) { /*-*/
        ...
        for (left = 0; left < (n - numMat + 1); left++) {
            ...
            for (c_index = left; c_index < right; c_index++) {
                ...
                c = CostSubTree(left, c_index) + CostSubTree(c_index + 1, right) + dims[left].height
                        * dims[c_index].width * dims[right].width;
    ...
    public void start() { /*-*/
        ...
        /*-*/frame.Highlight(line + 5);
        for (int i = 0; i < n - 1; i++)
            cost.setElem(i, i + 1, dims[i].height * dims[i].width * dims[i + 1].width);
        /*-*/frame.Highlight(line + 6);
        /*-*/frame.Highlight(line + 7);
        for (int k = 0; k < n - 1; k++)
    ...
    private void animateInit() {
        ...
        ComBox com = new ComBox(420, 20, "Initialization...", drawingPanel.getFixFont());
        drawingPanel.addCom(com);
        drawingPanel.repaint();
        drawingPanel.delay();

        ShadowLabel mesg1 = new ShadowLabel("There is only one possible way to multiply two");
        ...
        ShadowLabel mesg2 = new ShadowLabel("matrices in order...");
        ...
        ShadowLabel mesg3 = new ShadowLabel("The entries of the cost matrix are intialized.");
        ...
        mesg1.setText(mesg2.getText());
        mesg2.setText(mesg3.getText());
        ...
        FloatingBox fb = new FloatingBox("" + cost.elem(0, 1), drawingPanel.getFixFont());
        ...
        FloatingBox fb2 = new FloatingBox("" + dims[0].height, drawingPanel.getFixFont());
        ...
        FloatingBox fb3 = new FloatingBox("" + dims[0].width, drawingPanel.getFixFont());
        ...
        FloatingBox fb4 = new FloatingBox("" + dims[1].width, drawingPanel.getFixFont());
    ...
    private ComBox highlightDimMat(int left, int right) {
        ...
        ComBox com = new ComBox(420, 20, "Finding optimal order for these " + (right - left + 1)
                + " matrices product..", drawingPanel.getFixFont());
        drawingPanel.addCom(com);
    ...
    private DrawingObj[] animateCost(int left, int c_index, int right) {
        ...
        ComBox com = new ComBox(420, 70, "Checking this parenthesisation...", drawingPanel.getFixFont());
        drawingPanel.addCom(com);
        ...
        if (s1.length > 1) {
            ...
            DimIndicator d = new DimIndicator((p1.getX() + p1.getRight()) / 2, p1.getY() + 2, p1.getX(), p1.getRight(),
                    "cost[" + left + "," + c_index + "]", drawingPanel.getFixFont());
            drawingPanel.addDrawingObj(d);
            drawingPanel.repaint();
            drawingPanel.delay();

            fb1 = new FloatingBox("" + cost.elem(left, c_index), drawingPanel.getFixFont());
        ...
        if (s2.length > 1) {
            ...
            com.setText("The optimal cost for this obtained from cost matrix.");
            DimIndicator d = new DimIndicator((p2.getX() + p2.getRight()) / 2, p2.getY() + 2, p2.getX(), p2.getRight(),
                    "cost[" + (c_index + 1) + "," + right + "]", drawingPanel.getFixFont());
            drawingPanel.addDrawingObj(d);
            drawingPanel.repaint();
            drawingPanel.delay();

            fb2 = new FloatingBox("" + cost.elem(c_index + 1, right), drawingPanel.getFixFont());
        ...
        DimIndicator d1 = new DimIndicator((p1.getX() + p1.getRight()) / 2, p1.getY() + 2, p1.getX(), p1.getRight(), ""
                + dims[left].height + "x" + dims[c_index].width, drawingPanel.getFixFont());
        ...
        d1 = new DimIndicator((p2.getX() + p2.getRight()) / 2, p2.getY() + 2, p2.getX(), p2.getRight(), ""
                + dims[c_index + 1].height + "x" + dims[right].width, drawingPanel.getFixFont());
        drawingPanel.addDrawingObj(d1);
        drawingPanel.repaint();
        drawingPanel.delay();
        frame.waitStep();
        ...
        drawingPanel.removeObj(d1);
        com.setText("Cost = " + dims[left].height + "x" + dims[c_index + 1].height + "x" + dims[right].width + " = "
                + (dims[left].height * dims[c_index + 1].height * dims[right].width));
        drawingPanel.repaint();
        drawingPanel.delay();

        FloatingBox box = new FloatingBox("" + (dims[left].height * dims[c_index + 1].height * dims[right].width),
        ...
        box
                .setText(""
                        + (dims[left].height * dims[c_index].width * dims[right].width + CostSubTree(left, c_index) + CostSubTree(
                                c_index + 1, right)));
    ...
    private DrawingObj[] animateBest(DrawingObj[] objs, DrawingObj[] bestObjs) {
        ...
        ComBox com = new ComBox(30 + 3 * 66, 135, "This parenthesisation gives the best cost so far...", drawingPanel
                .getFixFont());
        drawingPanel.addCom(com);
        drawingPanel.repaint();
        drawingPanel.delay();
        ...
        if (bestObjs != null) {
            ComBox com2 = new ComBox(30 + 3 * 66 + 10, 145, "Remove previous best..", drawingPanel.getFixFont());
    ...
    private void animateRemoveObjs(DrawingObj[] objs) {
        ...
        ComBox com = new ComBox(objs[0].getX(), objs[0].getY() + 20, "Higher cost.. Dispose!", drawingPanel
                .getFixFont());
        drawingPanel.addCom(com);
        drawingPanel.repaint();
        drawingPanel.delay();
    ...
    private void animateStoreInMat(DrawingObj[] bestObjs, int left, int right, int bestIndex) {

        ComBox com = new ComBox(30 + 3 * 66, 135, "Moving optimal cost to cost matrix...", drawingPanel.getFixFont());
        drawingPanel.addCom(com);
        drawingPanel.repaint();
        drawingPanel.delay();
        ...
        traj.addElement(new Point(bestObjs[2].getX(), cost.getPosn(left, right).y));
        traj.addElement(new Point(cost.getPosn(left, right).x, cost.getPosn(left, right).y));
        drawingPanel.setAnimStep(10);
        drawingPanel.animate(bestObjs[2], traj);
        frame.waitStep();

        Parenthesis p1 = (Parenthesis) bestObjs[0];
        DimIndicator d = new DimIndicator((p1.getRight() - 7), p1.getY() + 2, p1.getRight() - 14, p1.getRight(), ""
        ...
        frame.waitStep();
        drawingPanel.removeObj(d);
        FloatingBox fb = new FloatingBox("" + bestIndex, drawingPanel.getFixFont());
    ...
    private void animateFormParen(Parenthesis left, Parenthesis right) {
        ...
        ComBox com = new ComBox(420, 50, "This parenthesis is obtained from best matrix...", drawingPanel.getFixFont());
        DimIndicator d = new DimIndicator(left.getRight() - 16, left.getY() + 2, left.getRight() - 23,
                left.getRight() - 9, "best[0," + (n - 1) + "]", drawingPanel.getFixFont());
        drawingPanel.addDrawingObj(d);
        drawingPanel.addCom(com);
        drawingPanel.repaint();
        drawingPanel.delay();
        FloatingBox fb = new FloatingBox("" + best.elem(0, 5), drawingPanel.getFixFont());
        ...
        d = new DimIndicator(left.getRight() - 16, left.getY() + 2, left.getRight() - 23, left.getRight() - 9,
