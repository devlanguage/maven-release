...
class FastConnectionFailover01 extends Thread {
    ...
    private void singleServiceDownUp() throws Exception {
        ...
        System.out.println("ConnectionFailoverEnabled: " + ds.getFastConnectionFailoverEnabled());
        verifyCache();
    ...
    private void handleDownEvent() throws Exception {
        ...
        body = "VERSION=1.0" + " service=" + service + " instance=" + instance + " database=" + database + " host=" + host + " status=DOWN";
    ...
    private void handleUpEvent() throws Exception {
        System.out.println("\n*** the service UP ***");
        body = "VERSION=1.0" + " service=" + service + " instance=" + instance + " database=" + database + " host=" + host + " status=up";
        if (sendEvent(type, comp, node, body))
            System.out.println("Service UP event is sent out");

    }

    private void getConnections(OracleDataSource ds, OracleConnection[] conns, int num) throws Exception {
        ...
        for (i = 0; i < num; ++i) {
            ...
            System.out.println("Conn[" + i + "] from Instance: " + instances[i] + "; Host: " + hosts[i] + "; Service: " + services[i]);
            rs.close();
            stmt.close();
        }
    ...
    private void verifyCache() throws Exception {
        System.out.println("NumberOfAvaibleConnections: " + occm.getNumberOfAvailableConnections("FCFCache01"));
        System.out.println("NumberOfActiveConnections: " + occm.getNumberOfActiveConnections("FCFCache01"));
    }
