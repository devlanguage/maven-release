...
public class UserDao {
	...
	/**
	 * 官方文档上还有例子: this.jdbcTemplate.update(
	 * "insert into t_actor (first_name, last_name) values (?, ?)","Leonor",
	 * "Watling"); this.jdbcTemplate.update(
	 * "update t_actor set = ? where id = ?","Banjo", 5276L);
	 * this.jdbcTemplate.update("delete from actor where id = ?"
	 * ,Long.valueOf(actorId)); 存储过程: this.jdbcTemplate.update(
	...
	public void doSaveObj(UserDM p) {
		 ...
		 * "insert into td_user(name,birthDay,sex,weight,height) values(" +
		 * "'"+p.getName()+"','"+new
		 * java.sql.Timestamp(p.getBirthday().getTime())+"',"+p.getSex()+","+p.
		 * getWeight()+","+p.getHeight()+")");
		 */
		// 插入方式二
		jdbcTemplate.update("insert into td_user(name,birthDay,sex,weight,height) values(?,?,?,?,?)",
				...
				new int[] { java.sql.Types.VARCHAR, java.sql.Types.TIMESTAMP, java.sql.Types.BOOLEAN,
						// java.sql.Types.DATE,则插入的只有日期,没有时间,2011-04-24
						// 00:00:00;TIMESTAMP:2011-04-24 19:09:24
	...
	public void doUpdateObj(UserDM p) {
		jdbcTemplate
				.update("update td_user set name = ? , birthDay = ? , sex = ? , weight = ? , height = ?  where id = ? ",
						new Object[] { p.getName(), p.getBirthday(), p.getSex(), p.getWeight(), p.getHeight(),
								p.getId() },
	...
	public int getCountEntites() {
		...
		// td_user");
		int rowCount = this.jdbcTemplate.queryForObject("select count(*) from td_user where id >= ?",
				new Object[] { 1 }, Integer.class);
		// select count(*) from td_user where id >= ?
		// = this.jdbcTemplate.queryForInt("select count(*) from td_user where
		// name = ?", "XXX");
		return rowCount;
	}
	...
	public Serializable getObjByID(int id) {
		// Query for a String
		// this.jdbcTemplate.queryForObject("select name from td_user where id =
		// ?",new Object[]{1212}, String.class);
		UserDM p = this.jdbcTemplate.queryForObject("select * from td_user where id = ?", new Object[] { id },
				new RowMapper<UserDM>() {
	...
	public List<UserDM> getObjsByID(int id) {
		List<UserDM> plist = this.jdbcTemplate.query("select * from td_user where id >= ?", new Object[] { id },
				new RowMapper<UserDM>() {
	...
	public String getNameAttri(int id) {
		String name = this.jdbcTemplate.queryForObject("select name from td_user where id = ?", new Object[] { id },
				String.class);
		return name;
	}

	public Date getBirthday(int id) {
		return this.jdbcTemplate.queryForObject("select birthDay from td_user where id = ?", new Object[] { id },
				Date.class);
	}
	...
	public int[] batchUpdate(final List<UserDM> Users) {
		int[] updateCounts = jdbcTemplate.batchUpdate("update td_user set name = ? where id = ?",
	...
	public int getNamedParameterJdbcCounts(int id) {
		...
		// 传一个对象
		// SqlParameterSource namedParametersx = new
		// BeanPropertySqlParameterSource(new User());
