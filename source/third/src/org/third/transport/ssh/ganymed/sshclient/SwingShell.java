 ...
 * 
 * - Verifying server hostkeys with an existing known_hosts file - Displaying fingerprints of server hostkeys - Adding a
 * server hostkey to a known_hosts file (+hashing the hostname for security) - Authentication with DSA, RSA, password
 * and keyboard-interactive methods
 *
 */
public class SwingShell {

    /*
     * NOTE: to get this feature to work, replace the "tilde" with your home directory, at least my JVM does not
     * understand it. Need to check the specs.
     */
    ...
    /**
     * This dialog displays a number of text lines and a text field. The text field can either be plain text or a
     * password field.
     */
    ...
    /**
     * TerminalDialog is probably the worst terminal emulator ever written - implementing a real vt100 is left as an
     * exercise to the reader, i.e., to you =)
     *
     */
    class TerminalDialog extends JDialog {
        ...
        public TerminalDialog(JFrame parent, String title, Session sess, int x, int y) throws IOException {
            ...
            /*
             * This is a hack. We cannot disable the caret, since setting editable to false also changes the meaning of
             * the TAB key - and I want to use it in bash. Again - this is a simple DEMO terminal =)
             */
    ...
    class AdvancedVerifier implements ServerHostKeyVerifier {
        public boolean verifyServerHostKey(String hostname, int port, String serverHostKeyAlgorithm, byte[] serverHostKey) throws Exception {
            ...
            if (choice == JOptionPane.YES_OPTION) {
                ...
                try {
                    KnownHosts.addHostkeyToFile(new File(knownHostPath), new String[] { hashedHostname }, serverHostKeyAlgorithm,
                            serverHostKey);
    ...
    /**
     * The SSH-2 connection is established in this thread. If we would not use a separate thread (e.g., put this code in
     * the event handler of the "Login" button) then the GUI would not be responsive (missing window repaints if you
     * move the window etc.)
     */
                ...
                while (true) {
                    if ((enableDSA || enableRSA) && conn.isAuthMethodAvailable(username, "publickey")) {
                        if (enableDSA) {
                            ...
                            if (key.exists()) {
                                EnterSomethingDialog esd = new EnterSomethingDialog(loginFrame, "DSA Authentication", new String[] {
                                        lastError, "Enter DSA private key password:" }, true);
                                esd.setVisible(true);
                        ...
                        if (enableRSA) {
                            ...
                            if (key.exists()) {
                                EnterSomethingDialog esd = new EnterSomethingDialog(loginFrame, "RSA Authentication", new String[] {
                                        lastError, "Enter RSA private key password:" }, true);
                                esd.setVisible(true);
                    ...
                    if (conn.isAuthMethodAvailable(username, "password")) {
                        final EnterSomethingDialog esd = new EnterSomethingDialog(loginFrame, "Password Authentication", new String[] {
