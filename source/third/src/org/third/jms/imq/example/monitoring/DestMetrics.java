...
/**
 * The DestMetrics example is a JMS application that monitors a destination on a Sun Java(tm) System Message Queue
 * broker. It does so by subscribing to a topic named: mq.metrics.destination.queue.<dest_name> OR
 * mq.metrics.destination.topic.<dest_name> Messages that arrive contain information describing the destination such as:
 * - number of messages that flowed into this destination - number of messages that flowed out of this destination -
 * size of message bytes that flowed into this destination - size of message bytes that flowed out of this destination -
 * etc.
 * 
 * Note that this example does not display all the information available in the destination metric message.
 * 
 * This application takes the following arguments: -t dest_type Specify required destination type. Valid values are 't'
 * or 'q'. -n dest_name Specify required destination name.
 * 
 * By default DestMetrics will connect to the broker running on localhost:7676. You can use -DimqAddressList attribute
...
public class DestMetrics implements MessageListener {
    ...
    public static void main(String args[]) {
        ...
        String destName = "QueueTest_1", destType = "q";
