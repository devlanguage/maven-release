...
/**
 * The BrokerMetrics example is a JMS application that monitors the Sun Java(tm) System Message Queue broker. It does so
 * by subscribing to a topic named 'mq.metrics.broker'. The messages that arrive contain Broker metric information such
 * as number of messages flowing in/out of the broker, size in bytes of messages flowing in/out etc.
 * 
 * This application also shows how to use the timestamp in each metric message to calculate rates e.g. rate at which
 * messages are flowing into the broker.
 * 
 * This application takes the following arguments: -m ttl Show totals e.g. total/accumulative number of msgs that flowed
 * in/out of the broker. This is the default. -m rts Show rates e.g. rate of mesasge flow in/out of broker.
 * 
 * By default BrokerMetrics will connect to the broker running on localhost:7676. You can use -DimqAddressList attribute
...
public class BrokerMetrics implements MessageListener {
    ...
    private long getRate(MapMessage previous, MapMessage mapMsg, float secs, String propName) throws JMSException {
