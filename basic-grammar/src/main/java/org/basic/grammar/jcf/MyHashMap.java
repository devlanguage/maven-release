 ...
 * operations, and permits <tt>null</tt> values and the <tt>null</tt> key. (The <tt>MyHashMap</tt> class is roughly
 ...
 * requires time proportional to the "capacity" of the <tt>MyHashMap</tt> instance (the number of buckets) plus its size
 ...
 * 
 * <p>
 * An instance of <tt>MyHashMap</tt> has two parameters that affect its performance: <i>initial capacity</i> and <i>load
 ...
 * <tt>MyHashMap</tt> class, including <tt>get</tt> and <tt>put</tt>). The expected number of entries in the map and its
 * load factor should be taken into account when setting its initial capacity, so as to minimize the number of rehash
 * operations. If the initial capacity is greater than the maximum number of entries divided by the load factor, no
 * rehash operations will ever occur.
 * 
 * <p>
 * If many mappings are to be stored in a <tt>MyHashMap</tt> instance, creating it with a sufficiently large capacity
 ...
 * 
 * <pre>
 *   Map m = Collections.synchronizedMap(new MyHashMap(...));
 * </pre>
 * 
 * <p>
...
public class MyHashMap<K, V> extends AbstractMap<K, V> implements Map<K, V>, Cloneable, Serializable {
  ...
  /**
   * The number of times this MyHashMap has been structurally modified Structural modifications are those that change
   * the number of mappings in the MyHashMap or otherwise modify its internal structure (e.g., rehash). This field is
   * used to make iterators on Collection-views of the MyHashMap fail-fast. (See ConcurrentModificationException).
   */
  ...
  /**
   * Constructs an empty <tt>MyHashMap</tt> with the specified initial capacity and the default load factor (0.75).
   * 
   * @param initialCapacity
   *          the initial capacity.
   * @throws IllegalArgumentException
  ...
  /**
   * Constructs an empty <tt>MyHashMap</tt> with the default initial capacity (16) and the default load factor (0.75).
   */
  public MyHashMap() {
  ...
  /**
   * Constructs a new <tt>MyHashMap</tt> with the same mappings as the specified <tt>Map</tt>. The <tt>MyHashMap</tt> is
   ...
   * readObject) after MyHashMap has been initialized but before any entries have been inserted. (In the absence of this
   ...
   * is critical because MyHashMap uses power-of-two length hash tables, that otherwise encounter collisions for
  ...
  /**
   * Returns the entry associated with the specified key in the MyHashMap. Returns null if the MyHashMap contains no
   * mapping for the key.
   */
   ...
   *         <tt>key</tt>.)
   */
  static Set<Integer> bucketIndex = new HashSet<>();
  public V put(K key, V value) {
    if (key == null)
      return putForNullKey(value);
    int hash = hash(key.hashCode());
    int i = indexFor(hash, table.length);
    System.out.println("hash="+hash+",bucketIndex="+i);
    if(bucketIndex.contains(i)){
      System.out.println("Collision_"+i);
    }
    bucketIndex.add(i);
   ...
   * 
   * @param newCapacity
   *          the new capacity, MUST be a power of two; must be greater than current capacity unless current capacity is
  ...
  public void putAll(Map<? extends K, ? extends V> m) {
    ...
    /*
     * Expand the map if the map if the number of mappings to be added is greater than or equal to threshold. This is
     * conservative; the obvious condition is (m.size() + size) >= threshold, but this condition could result in a map
     * with twice the appropriate capacity, if the keys to be added overlap with the keys already in this map. By using
     * the conservative calculation, we subject ourself to at most one extra resize.
     */
  ...
  /**
   * Removes and returns the entry associated with the specified key in the MyHashMap. Returns null if the MyHashMap
  ...
  /**
   * Returns a shallow copy of this <tt>MyHashMap</tt> instance: the keys and values themselves are not cloned.
   * 
  ...
  static class Entry<K, V> implements Map.Entry<K, V> {
     ...
     * that's already in the MyHashMap.
     */
   ...
   * 
   * Subclass overrides this to alter the behavior of MyHashMap(Map), clone, and readObject.
   */
   ...
   * map, via the <tt>Iterator.remove</tt>, <tt>Collection.remove</tt>, <tt>removeAll</tt>, <tt>retainAll</tt> and
   ...
   * map are reflected in the set, and vice-versa. If the map is modified while an iteration over the set is in progress
   * (except through the iterator's own <tt>remove</tt> operation, or through the <tt>setValue</tt> operation on a map
   * entry returned by the iterator) the results of the iteration are undefined. The set supports element removal, which
   ...
   * 
   * @serialData The <i>capacity</i> of the MyHashMap (the length of the bucket array) is emitted (int), followed by the
   *             <i>size</i> (an int, the number of key-value mappings), followed by the key (Object) and value (Object)
  ...
  /**
   * Reconstitute the <tt>MyHashMap</tt> instance from a stream (i.e., deserialize it).
   */
