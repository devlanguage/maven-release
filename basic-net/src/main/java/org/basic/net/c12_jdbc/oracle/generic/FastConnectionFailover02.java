...
class FastConnectionFailover02 extends Thread {
    ...
    public static void main(String args[]) throws SQLException {
        FastConnectionFailover02 fcf02 = new FastConnectionFailover02();
        fcf02.start();
    }
    ...
    private void oneServiceDownOneServiceUp() throws Exception {
        ...
        System.out.println("\nNow we can retrieve connections from " + "cache FCFCacheGL again.");
        getConnections(dsgl, conns1, 5);
        verifyCache();
    ...
    private void getURLs() throws Exception {
        ...
        if ((url != null) && (url.indexOf("thin") != -1)) {
            urlgl = "jdbc:oracle:thin:@(DESCRIPTION=" + "(FAILOVER=on)" + "(ADDRESS=(PROTOCOL=tcp)(HOST=localhost)(PORT=1251))"
                            + "(CONNECT_DATA=(SERVICE_NAME=gl.oracle.com)))";
            urlcrm = "jdbc:oracle:thin:@(DESCRIPTION=" + "(FAILOVER=on)" + "(ADDRESS=(PROTOCOL=tcp)(HOST=localhost)(PORT=1255))"
                            + "(CONNECT_DATA=(SERVICE_NAME=crm.oracle.com)))";
        }
        else {
            urlgl = "jdbc:oracle:oci8:@(DESCRIPTION=" + "(FAILOVER=on)" + "(ADDRESS=(PROTOCOL=tcp)(HOST=localhost)(PORT=1251))"
                            + "(CONNECT_DATA=(SERVICE_NAME=gl.oracle.com)))";
            urlcrm = "jdbc:oracle:oci8:@(DESCRIPTION=" + "(FAILOVER=on)" + "(ADDRESS=(PROTOCOL=tcp)(HOST=localhost)(PORT=1255))"
                            + "(CONNECT_DATA=(SERVICE_NAME=crm.oracle.com)))";
        }
    } // end of getURLs()

    private OracleDataSource createDataSource(String cacheName, String url) throws Exception {
    ...
    private void cleanUp(Connection[] conns1, Connection[] conns2, int num, OracleDataSource ods1, OracleDataSource ods2) throws Exception {
        int i = 0;
        for (i = 0; i < num; ++i) {
            if (conns1[i] != null) {
                conns1[i].close();
    ...
    private void printFailoverSettings(OracleDataSource dsgl, OracleDataSource dscrm) throws Exception {
        System.out.println("DataSource for service gl " + "ConnectionFailoverEnabled: " + dsgl.getFastConnectionFailoverEnabled());
        System.out.println("DataSource for service crm " + "ConnectionFailoverEnabled: " + dscrm.getFastConnectionFailoverEnabled());
    }

    private void getConnections(OracleDataSource ds, Connection[] conns, int num) throws Exception {
        for (int i = 0; i < num; ++i) {
            ...
            rs.next();
            System.out.println("conns[" + i + "] from Instacne: " + rs.getString(1) + "; Host: " + rs.getString(2) + "; Service: "
                            + rs.getString(3));
            rs.close();
            stmt.close();
        }
    ...
    private void verifyCache() throws Exception {
        System.out.println("NumberOfAvaibleConnections in cache FCFCacheGL: " + occm.getNumberOfAvailableConnections("FCFCacheGL"));
        System.out.println("NumberOfActiveConnections in cache FCFCacheGL: " + occm.getNumberOfActiveConnections("FCFCacheGL"));
        System.out.println("NumberOfAvaibleConnections in cache FCFCacheCRM: " + occm.getNumberOfAvailableConnections("FCFCacheCRM"));
        System.out.println("NumberOfActiveConnections in cache FCFCacheCRM: " + occm.getNumberOfActiveConnections("FCFCacheCRM"));
    }
    ...
    private void handleDownEvent() throws Exception {
        ...
        body = "VERSION=1.0" + " service=gl.oracle.com" + " instance=myinstance" + " database=myinstance" + " host=myhost" + " status=DOWN";
    ...
    private void handleUpEvent() throws Exception {
        System.out.println("\n*** the service UP ***");
        type = "database/event/service";
        body = "VERSION=1.0" + " service=gl.oracle.com" + " instance=myinstance" + " database=myinstance" + " host=myhost" + " status=up";
        if (sendEvent(type, comp, node, body))
            System.out.println("Service UP event is sent out");

    }

    private boolean sendEvent(String type, String comp, String node, String body) throws Exception {
        try {
            System.out.println("Send out Event: type=" + type + " comp=" + comp + " node=" + node + " body=" + body);

            // create a Publisher object for pulishing notifiactions
            Publisher p = new Publisher("generator");

            // create the notification body
            Notification anotification = new Notification(type, comp, node, body.getBytes());
